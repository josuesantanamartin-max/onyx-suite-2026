...
const NetWorthCard: React.FC<NetWorthCardProps> = ({ accounts, debts, monthlyIncome, monthlyExpenses, onNavigate }) => {
    ...
    return (
        <div onClick={() => onNavigate('finance', 'dashboard')} className="col-span-1 md:col-span-12 lg:col-span-8 bg-white p-10 rounded-onyx border border-onyx-100 shadow-sm hover:shadow-xl transition-all duration-500 cursor-pointer group relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-12 relative z-10">
                <div>
                    <div className="flex items-center gap-3 mb-3">
                        <div className="p-2 bg-onyx-50 text-onyx-600 rounded-lg group-hover:bg-indigo-50 group-hover:text-indigo-primary transition-colors"><Wallet className="w-5 h-5" /></div>
                        <span className="text-[10px] font-bold text-onyx-400 uppercase tracking-[0.2em]">Patrimonio Global</span>
                    </div>
                    <div className="flex items-baseline gap-4 mt-2">
                        <h3 className="text-5xl font-bold text-onyx-950 tracking-tight">{formatEUR(netWorth)}</h3>
                        {cashFlow > 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 relative z-10">
                <div className="p-6 bg-onyx-50/50 border border-onyx-100/30 rounded-2xl group-hover:bg-white group-hover:border-onyx-100 group-hover:shadow-md transition-all duration-500">
                    <p className="text-[10px] font-bold text-onyx-400 uppercase tracking-[0.15em] mb-3 flex items-center gap-2.5">
                        <Coins className="w-3.5 h-3.5 opacity-40" />
                        Liquidez
                    </p>
                    <p className="text-2xl font-bold text-onyx-900 tracking-tight">{formatEUR(liquidAssets)}</p>
                </div>
                <div className="p-6 bg-onyx-50/50 border border-onyx-100/30 rounded-2xl group-hover:bg-white group-hover:border-onyx-100 group-hover:shadow-md transition-all duration-500">
                    <p className="text-[10px] font-bold text-onyx-400 uppercase tracking-[0.15em] mb-3 flex items-center gap-2.5">
                    ...
                    <p className="text-2xl font-bold text-onyx-900 tracking-tight">{formatEUR(investedAssets)}</p>
                </div>
                <div className="p-6 bg-onyx-50/50 border border-onyx-100/30 rounded-2xl group-hover:bg-white group-hover:border-onyx-100 group-hover:shadow-md transition-all duration-500">
                    <p className="text-[10px] font-bold text-onyx-400 uppercase tracking-[0.15em] mb-3 flex items-center gap-2.5 text-red-400">
